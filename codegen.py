...
class CodeGenerator:
    ...
    def _step(self, elem: RpnElement):
        ...
        # ── ТИП — описание переменных ────────────────────────
            ...
            # В R нет явных объявлений — генерируем комментарий
            vars_list = []
            for _ in range(count):
                vars_list.insert(0, self._pop())
            snap['code_line'] = f"# var {', '.join(vars_list)}"
            self._emit_comment(f"Переменные: {', '.join(vars_list)}")

        # ── КО — конец описания ──────────────────────────────
        elif kind == 'КО':
            # В R ничего не генерируем
            pass
